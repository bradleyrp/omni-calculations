...
import ortho
from ortho import str_types,requires_python
from omni import basic_compute_loop,framelooper

from omni import get_automacs,status

vecnorm = lambda vec: vec/np.linalg.norm(vec)
vecangle = lambda v1,v2 : np.arccos(np.dot(vecnorm(v1),vecnorm(v2)))*(180./np.pi)

@requires_python('sklearn')
def hydrogen_bonding(structure,trajectory,**kwargs):
	"""
	Generic hydrogen bonding code.
	Revamped on 2017.4.28 to generate a more uniform data structure.
	"""
	# put requirements here and decorate with require_python
	# note that importing just-in-time is useful for non-standard packages
	import sklearn
	...
	donors_heavy,donors_h = [uni.select_atoms(' or '.join(['(resname %s and (%s))'%(
		resname if isinstance(resname,str_types) else resname[1],' or '.join(['name %s'%i[w] 
	...
	acceptors_heavy = uni.select_atoms(' or '.join(['(resname %s and (%s))'%(
		resname if isinstance(resname,str_types) else resname[1],' or '.join(['name %s'%i 
	...
	incoming = basic_compute_loop(hbonds.hbonder_framewise,looper,run_parallel=run_parallel)

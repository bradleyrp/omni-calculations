...
from ortho import status,time_limit,TimeoutException
...
def topologize(pos,vec,tol=None):
	"""
	...
	belligerent systems. See RPB notes for more details. Modified on 2018.09.28 to fix a serious bug in 
	which the shifts were not updating. Note that this means that if a few atoms drag a leaflet over to the
	other side
	"""
	...
	#! fixing: changed move_votes[0] to move_votes[-1]
	while (np.sum(np.abs(move_votes[-1])>len(pos)/2)>len(pos)*tol or step == 0) and step<100:
		print('TOPOLOGIZE ITERATION: step=%d sum=%.3f len(pos)=%d tol=%.3f val=%.3f'%(
			step,np.sum(np.abs(move_votes[-1])>len(pos)/2),len(pos),tol,len(pos)*tol))
		...
		for ind,bead in enumerate(pos):
			...
			for d in range(3):
				...
				adds_high[np.where(np.all((pd[d][ind]>vec[d]/2.,pos[ind,d]<pos[:,d]),axis=0))] = 1
				adds_low[np.where(np.all((pd[d][ind]>vec[d]/2.,pos[ind,d]>pos[:,d]),axis=0))] = -1
				# for each spatial dimension, we tally votes to move the point by a box vector
				move_votes[step][:,d] += adds_high+adds_low
		# fixing: applying the move_votes
		#! kp = np.array(pos)
		kp = pos + np.round(move_votes[-1]/(natoms/2.))*vec
		step += 1
		#!move_votes = np.concatenate((move_votes,np.zeros((1,natoms,3))))
	...
	recall that the previous solution was to first calculate a set of "move votes"
	each move vote in each dimension is nonzero positive or negative at positions where
	the distance at one dimension to all the other points is greater than the half-box-vector
	or at least that is how it seems from the code above
	move votes at each step appear to accumulate
	later they are summed and returned
	leaving open the obvious possiblity that they could simply oscillate
	"""
	return moved

class LeafletFinderREDEV:
...
class LeafletFinder:
	...
	def identify_leaflets_cluster(self,pts,vec,topologize_time_limit=30,max_count_asymmetry=0.05):
		...
		else: #except TimeoutException, msg: 
			status('topologize failed to join the bilayer. '
				'if it is broken over PBCs e.g. a saddle, this is a serious error which may go undetected. '
				'make sure you always inspect the topology later.',tag='error')
			wrapper = np.zeros((len(pts),3))
		findframe = pts + wrapper*np.array(vec)
		# previously "ensure that all points are in the box"
		# previously: findframe += vec*(findframe<0) - vec*(findframe>vec)
		# note that the above was a serious error. topologize attempts to unwrap PBCs for a bilayer which is
		#   not a sure thing in contrast to unwrapping PBCs for molecules because the bilayer is continuous
		#   accross boundaries. it is generally assured of putting small chunks of the bilayer back with the
		#   right leaflet, however a tortured enough shape might cause the leaflets to flop. either way, it
		#   is a bad idea to "put things in the box" because this is basically the opposite of the PBC 
		#   operation which we call topologize here, as in "make a coherent topology" for this object
		...
		# fast, normal method. see the docs for description of percolation
		if True:
			# got function object has no attribute 'index' here once from 
			#   "sparse/sputils.py", line 281 in scipy
			# this is actually a common problem but you can just rerun
			...
			ward = sklearn.cluster.AgglomerativeClustering(n_clusters=2,
				connectivity=connectivity,linkage='average').fit(findframe)
		# no connectivity means it goes slower
		else:
			ward = sklearn.cluster.AgglomerativeClustering(n_clusters=2,
				linkage='single').fit(findframe)
		# debug for review-lipid_abstractor.py function
		self.debug_findframe = findframe
		self.debug_pts = pts
		self.debug_wrapper = wrapper
		self.debug_vec = vec
		imono = ward.labels_
		if np.mean(imono)==0.5: 
			status('[STATUS] perfect split is %0.5f'%np.mean(imono))
			...
			np.abs(np.mean(imono)-0.5)>=max_count_asymmetry):
			# the following error should basically never happen
			status('[STATUS] split is %0.5f'%np.mean(imono))
			status('[STATUS] one side has %d'%np.sum(imono))
			status('[WARNING] leaflets were not distinguished')
			if False: raise Exception('[ERROR] failed to identify leaflets. '
				'DEVELOPMENT NOTE. this method is pretty robust. check PBCS.')
		else: status('[STATUS] some lipids might be flipped %d %.5f'%(np.sum(imono),np.mean(imono)))

...
import numpy as np
import MDAnalysis
from codes import mesh
from omni import checktime,basic_compute_loop,status
...
def lipid_abstractor(grofile,trajfile,**kwargs):
	...
	arrest = kwargs.pop('arrest',False)
	sn = kwargs['sn']
	...
	# parallel compute
	looper = [dict(coords=trajectory[fr],masses=masses,divider=divider) for fr in range(nframes)]
	coms = basic_compute_loop(mesh.centroid,looper,run_parallel=parallel)
	if arrest=='coms': return coms
	...
	leaflet_finder = mesh.LeafletFinder(
	...
	if arrest=='leaflet_finder_fail': 
		return dict(leaflet_finder=leaflet_finder,selected_frames=selected_frames,vecs=vecs,
			atoms_separator=atoms_separator,coms=coms)
	# get the indices from the leaflet finder
	monolayer_indices = leaflet_finder.monolayer_indices

	# check the right leaflet split. probably easier to check this in a downstream step
	if 'split' in separator:
		split_factor = separator['split']
		leaflet_split = tuple([float(sum(monolayer_indices==i)) for i in [0,1]])
		leaflet_ratio = float(leaflet_split[1]/(leaflet_split[1]+leaflet_split[0]))
		if isinstance(separator['split'],tuple) and len(split_factor)==2:
			if not any([leaflet_split==split_factor[::k] for k in [1,-1]]):
				raise Exception('leaflet split %s does not match observed %s'%(
					str(leaflet_split),str(split_factor)))
		elif isinstance(split_factor,float):
			if leaflet_ratio!=split_factor and 1./leaflet_ratio!=split_factor:
				raise Exception('expected leaflet split %.4f and we observed %.4f'%(
					split_factor,leaflet_ratio))
		else: raise Exception('invalid split: %s'%str(split_factor))
